...
**Question:**
A neuron has a transfer function which is a linear weighted sum of its inputs. The weights are $w = [0.1, -0.5, 0.4]$. Calculate the net input (before any activation function is applied) for the following input vector:
$x_1 = [0.1, -0.5, 0.4]^T$
...
1.  **Identify the vectors:**
      ...
      * Input ($x_1$): $[0.1, -0.5, 0.4]$
        Here, the first element usually corresponds to the bias input (if $x_0=1$ is assumed) or simply the first feature. We align them element by element.

2.  **Multiply corresponding elements (Element-wise multiplication):**

      * $w_1 \times x_{1,1} = 0.1 \times 0.1 = 0.01$
      * $w_2 \times x_{1,2} = -0.5 \times -0.5 = 0.25$
      * $w_3 \times x_{1,3} = 0.4 \times 0.4 = 0.16$

3.  **Sum the results (Dot Product):**
...
Let's apply a threshold function to calculate a neuron's output.

**Question:**
A neuron has a transfer function which is a linear weighted sum of its inputs and an activation function that is the Heaviside function.

  * Weights: $w = [0.1, -0.5, 0.4]$ (Bias is implicit or zero for this context)
  * Threshold: 0
  * Input 1: $x_1 = [0.1, -0.5, 0.4]^T$
  * Input 2: $x_2 = [0.1, 0.5, 0.4]^T$

Calculate the output for both inputs.

**Step-by-Step Calculation:**

1.  **Calculate Net Input ($s$) for $x_1$:**
...
2.  **Apply Activation Function for $x_1$:**
    Since $s_1 (0.42) \ge 0$, the Heaviside function outputs 1.

3.  **Calculate Net Input ($s$) for $x_2$:**
...
4.  **Apply Activation Function for $x_2$:**
    Since $s_2 (-0.08) < 0$, the Heaviside function outputs 0.
...
Let's determine the class of a specific data point using a given weight vector and bias.

**Question:**
Consider a two-category classification problem. The discriminant function is defined by the weight vector $w = [2, 3]^T$ and a bias $w_0 = -6$.
Determine the class for the input vector $x = [2, 1]^T$.

**Step-by-Step Calculation:**

1.  **Identify the parameters:**

      * Weight vector $w = [2, 3]^T$
      * Input vector $x = [2, 1]^T$
      * Bias $w_0 = -6$

2.  **Calculate the dot product ($w^T x$):**
    Multiply corresponding elements of $w$ and $x$ and sum them.

    $$w^T x = (2 \times 2) + (3 \times 1)$$
    $$w^T x = 4 + 3$$
    $$w^T x = 7$$

3.  **Add the bias ($w_0$) to find $g(x)$:**
...
4.  **Apply the Classification Rule:**
    We compare the result to 0.
    Since $g(x) = 1$ and $1 > 0$, we classify this input as **Class 1**.
...
**Question:**
A decision boundary is defined by the discriminant function $g(x) = 4x_1 + 3x_2 - 12$.
Calculate the algebraic distance $r$ from the point $x = [4, 2]^T$ to the decision boundary.

**Step-by-Step Calculation:**

1.  **Identify the parameters:**

      * Weight vector $w$: From $4x_1 + 3x_2$, we get $w = [4, 3]^T$.
      * Bias $w_0$: $-12$.
      * Input vector $x$: $[4, 2]^T$.

2.  **Calculate the value of the discriminant function $g(x)$:**
    Substitute the input values into the equation.

    $$g(x) = (4 \times 4) + (3 \times 2) - 12$$
    $$g(x) = 16 + 6 - 12$$
    $$g(x) = 10$$

3.  **Calculate the magnitude of the weight vector $||w||$:**
    Use the Pythagorean theorem ($\sqrt{w_1^2 + w_2^2}$).
...
Let's convert a standard problem into augmented notation and verify the result is the same.

**Question:**
Given weights $w = [3, -2]^T$, bias $w_0 = 4$, and input $x = [2, 5]^T$, calculate $g(x)$ using augmented vector notation.

**Step-by-Step Calculation:**

1.  **Construct the augmented input vector ($y$):**
    Prepend the value $1$ to the vector $x$.
    $$y = [1, 2, 5]^T$$

2.  **Construct the augmented weight vector ($a$):**
    Prepend the bias $w_0$ to the vector $w$.
    $$a = [4, 3, -2]^T$$

3.  **Calculate the dot product ($a^T y$):**
    Multiply corresponding elements and sum them.

    $$g(x) = (4 \times 1) + (3 \times 2) + (-2 \times 5)$$
    $$g(x) = 4 + 6 - 10$$
    $$g(x) = 0$$
...
**Computational Example: Error Correction**

The following example demonstrates one epoch (one pass through the training data) of the error correction method using a Heaviside step function.

**Question:**
Consider the following set of training data. Compute the results of using the error-correction method on these examples.
...
**Parameters:**
  ...
  * **Initial Weights:** $w_1 = 1$, $w_2 = 2$.
  * **Bias:** The bias input is always $1$ ($x_0 = 1$); Initial bias weight $w_0 = 1$.
  * **Learning Rate ($\alpha$):** 0.5.
  * **Transfer Function:** Step function ($y = 1$ if $s \ge 0$, else $0$).

**Step-by-Step Calculation:**

**1. First Training Example: $x = [1, 1, 1]^T$ (including bias), $t = 1$**

First, we calculate the weighted sum ($s$):
$$s = (w_0 \times x_0) + (w_1 \times x_1) + (w_2 \times x_2)$$
$$s = (1 \times 1) + (1 \times 1) + (2 \times 1) = 4$$

We apply the step function. Since $4 \ge 0$, the output $y = 1$.
Comparing to the target: $t = 1$ and $y = 1$. The prediction is correct.
**Update:** No change.

  * Current weights: $w = [1, 1, 2]$.

**2. Second Training Example: $x = [1, 3, 1]^T$, $t = 0$**

Calculate the weighted sum:
$$s = (1 \times 1) + (1 \times 3) + (2 \times 1) = 6$$

Apply step function. Since $6 \ge 0$, output $y = 1$.
Comparing to the target: $t = 0$ and $y = 1$. The prediction is **incorrect** (output too high). We must adjust weights.

Update rule: $w_i \leftarrow w_i + 0.5(0 - 1)x_i \Rightarrow w_i - 0.5x_i$.

  * $w_0 \leftarrow 1 - 0.5(1) = 0.5$.
  * $w_1 \leftarrow 1 - 0.5(3) = -0.5$.
  * $w_2 \leftarrow 2 - 0.5(1) = 1.5$.

**New weights:** $w = [0.5, -0.5, 1.5]$.

**3. Third Training Example: $x = [1, 1, 1]^T$, $t = 1$**

Calculate sum using new weights:
...
**4. Fourth Training Example: $x = [1, 0, 2]^T$, $t = 0$**

Calculate sum:
$$s = (0.5 \times 1) + (-0.5 \times 0) + (1.5 \times 2)$$
$$s = 0.5 + 0 + 3 = 3.5$$

Apply step function. Since $3.5 \ge 0$, output $y = 1$.
Comparing to target: $t = 0$ and $y = 1$. The prediction is **incorrect**.

Update rule: $w_i \leftarrow w_i + 0.5(0 - 1)x_i$

  * $w_0 \leftarrow 0.5 - 0.5(1) = 0$
  * $w_1 \leftarrow -0.5 - 0.5(0) = -0.5$
...
**Question:**
Use the batch perceptron learning algorithm with augmented notation and sample normalisation to find parameters for the linear discriminant function. Assume initial values $a^T = (-25, 6, 3)$ and a learning rate $\eta = 1$. The data set (and its normalized augmented form $y$) is:

  * Sample 1: $y_1 = (1, 1, 5)^T$
  * Sample 2: $y_2 = (1, 2, 5)^T$
  * Sample 3: $y_3 = (-1, -4, -1)^T$
  * Sample 4: $y_4 = (-1, -5, -1)^T$

*(Note: These normalized vectors are derived from the raw data where some samples belong to Class 2 and are thus negated)*.

**Step-by-Step Calculation (Epoch 1):**

1.  **Check for Misclassification:**
    We calculate $g(x) = a^T y$ for each sample. A sample is misclassified if $g(x) \le 0$ (since we are using normalized targets).

      * **Sample 1 ($y_1$):**
        $g(x) = (-25 \times 1) + (6 \times 1) + (3 \times 5) = -25 + 6 + 15 = -4$.
        Result is $\le 0$, so **Misclassified**.

      * **Sample 2 ($y_2$):**
        $g(x) = (-25 \times 1) + (6 \times 2) + (3 \times 5) = -25 + 12 + 15 = 2$.
        Result is $> 0$, so **Correct**.

      * **Sample 3 ($y_3$):**
        $g(x) = (-25 \times -1) + (6 \times -4) + (3 \times -1) = 25 - 24 - 3 = -2$.
        Result is $\le 0$, so **Misclassified**.

      * **Sample 4 ($y_4$):**
        $g(x) = (-25 \times -1) + (6 \times -5) + (3 \times -1) = 25 - 30 - 3 = -8$.
        Result is $\le 0$, so **Misclassified**.
...
3.  **Update the Weights:**
    $$a \leftarrow a + \eta \times Sum$$
    $$a \leftarrow \begin{pmatrix} -25 \\ 6 \\ 3 \end{pmatrix} + 1 \times \begin{pmatrix} -1 \\ -8 \\ 3 \end{pmatrix}$$
    $$a \leftarrow \begin{pmatrix} -26 \\ -2 \\ 6 \end{pmatrix}$$
...
**Computational Example: Sequential Updates**

Let's trace the first epoch of sequential learning for a specific dataset.

**Question:**
Apply the sequential perceptron learning algorithm to determine the parameters of a linear discriminant function.
**Data:**

  * Class 1 ($1$): $[0,0]^T, [1,0]^T, [2,1]^T$
  * Class 2 ($-1$): $[0,1]^T, [1,2]^T$

**Parameters:**

  * Initial weights: $a = [-1.5, 5, -1]^T$ (Augmented vector $[w_0, w_1, w_2]$)
  * Learning rate $\eta = 1$

**Step-by-Step Calculation:**

**1. Sample 1:** $x = [0, 0]^T$, Class 1.

  * **Augmented Input ($y$):** $[1, 0, 0]^T$

  * **Calculate Output:**
    $$g(x) = a^T y = (-1.5 \times 1) + (5 \times 0) + (-1 \times 0) = -1.5$$

  * **Check:** Output is negative, but Class 1 requires positive ($>0$). **Misclassified.**

  * **Update:** Move weights *towards* the input (Add $y$).
    $$a_{new} = [-1.5, 5, -1] + [1, 0, 0] = [-0.5, 5, -1]$$

    ```mermaid
    graph LR
    OldW[Old Weight] -- "+ Learning Rate * Input" --> NewW[New Weight]
    style NewW fill:#9f9,stroke:#333
    ```

**2. Sample 2:** $x = [1, 0]^T$, Class 1.

  * **Augmented Input ($y$):** $[1, 1, 0]^T$
  * **Calculate Output (using new weights):**
    $$g(x) = (-0.5 \times 1) + (5 \times 1) + (-1 \times 0) = 4.5$$
  * **Check:** Output is $4.5 (>0)$. Class 1 requires positive. **Correct.**
  * **Update:** No change. Weights remain $[-0.5, 5, -1]$.

**3. Sample 3:** $x = [2, 1]^T$, Class 1.

  * **Augmented Input ($y$):** $[1, 2, 1]^T$
  * **Calculate Output:**
    $$g(x) = (-0.5 \times 1) + (5 \times 2) + (-1 \times 1) = -0.5 + 10 - 1 = 8.5$$
  * **Check:** Output is $8.5 (>0)$. **Correct.**
  * **Update:** No change. Weights remain $[-0.5, 5, -1]$.

**4. Sample 4:** $x = [0, 1]^T$, Class 2 ($-1$).

  * **Augmented Input ($y$):** $[1, 0, 1]^T$
  * **Calculate Output:**
    $$g(x) = (-0.5 \times 1) + (5 \times 0) + (-1 \times 1) = -1.5$$
  * **Check:** Output is $-1.5 (<0)$. Class 2 requires negative. **Correct.**
  * **Update:** No change. Weights remain $[-0.5, 5, -1]$.

**5. Sample 5:** $x = [1, 2]^T$, Class 2 ($-1$).

  * **Augmented Input ($y$):** $[1, 1, 2]^T$

  * **Calculate Output:**
    $$g(x) = (-0.5 \times 1) + (5 \times 1) + (-1 \times 2) = -0.5 + 5 - 2 = 2.5$$

  * **Check:** Output is $2.5 (>0)$, but Class 2 requires negative. **Misclassified.**

  * **Update:** Move weights *away* from the input (Subtract $y$).
    $$a_{new} = [-0.5, 5, -1] - [1, 1, 2] = [-1.5, 4, -3]$$

    ```mermaid
    graph LR
    OldW[Old Weight] -- "- Learning Rate * Input" --> NewW[New Weight]
    style NewW fill:#f99,stroke:#333
    ```

**Result after one epoch:** The updated weight vector is $a = [-1.5, 4, -3]^T$. The algorithm would then cycle through the data again until no errors are found.

## 3.5 - Computational Examples of Perceptron Updates

To solidify the understanding of these algorithms, we will walk through manual computations for both the Batch and Sequential methods.

**Example 1: Batch Perceptron Learning**

**Problem Statement**
Consider a dataset used to train a linear discriminant function $g(x) = w^T x + w_0$. We define the classification rule: Assign to Class 1 if $g(x) > 0$, and Class 2 otherwise. We will use the **Batch Perceptron Learning Algorithm** with **augmented notation** and **sample normalization**.

  * **Initial Weights:** $a = (-25, 6, 3)^T$ (where $a$ includes the bias weight $w_0$).
  * **Learning Rate:** $\eta = 1$.
  * **Training Data:**

| Sample | Feature Vector $x^T$ | Class |
| :----- | :------------------- | :---- |
| 1      | (1, 5)               | 2     |
| 2      | (2, 5)               | 1     |
| 3      | (4, 1)               | 1     |
...
1.  **Augmentation and Normalization:**
    We convert the feature vectors into augmented vectors $y$ (adding 1 as the first element). Since we are using sample normalization, we negate the vectors belonging to Class 2. This allows us to use the single condition $a^T y > 0$ for correct classification.
      ...
      * **Sample 1:** Class 2. Augment: $(1, 1, 5)$. Negate: $y_1 = (-1, -1, -5)$.
      * **Sample 2:** Class 1. Augment: $(1, 2, 5)$. Keep: $y_2 = (1, 2, 5)$.
      * **Sample 3:** Class 1. Augment: $(1, 4, 1)$. Keep: $y_3 = (1, 4, 1)$.
      * **Sample 4:** Class 2. Augment: $(1, 5, 1)$. Negate: $y_4 = (-1, -5, -1)$.
...
2.  **Check for Misclassification:**
    We calculate the dot product $g(x) = a^T y$ for each sample using the initial weights $a = (-25, 6, 3)$. A result $\le 0$ indicates an error.
      ...
      * **Sample 1 ($y_1$):** $(-25 \times -1) + (6 \times -1) + (3 \times -5) = 25 - 6 - 15 = 4$. (**Correct**, $>0$)
      * **Sample 2 ($y_2$):** $(-25 \times 1) + (6 \times 2) + (3 \times 5) = -25 + 12 + 15 = 2$. (**Correct**, $>0$)
      * **Sample 3 ($y_3$):** $(-25 \times 1) + (6 \times 4) + (3 \times 1) = -25 + 24 + 3 = 2$. (**Correct**, $>0$)
      * **Sample 4 ($y_4$):** $(-25 \times -1) + (6 \times -5) + (3 \times -1) = 25 - 30 - 3 = -8$. (**Misclassified**, $\le 0$)

3.  **Update Weights:**
    In Batch learning, we sum the vectors of *all* misclassified samples to compute the update. Here, only Sample 4 ($y_4$) is misclassified.

    $$a_{new} = a_{old} + \eta \sum_{y \in \mathcal{Y}} y$$
    $$a_{new} = (-25, 6, 3) + 1 \times (-1, -5, -1)$$
    $$a_{new} = (-26, 1, 2)$$

After one epoch, the weights are updated to $(-26, 1, 2)$. The decision boundary would shift to reduce the error on Sample 4.

```mermaid
graph LR
    Init[Initial: -25, 6, 3] -- Epoch 1 (Sample 4 Error) --> End[Updated: -26, 1, 2]
...
**Problem Statement**
We will now use the **Sequential Perceptron Learning Algorithm** on a different linearly separable dataset. We update the weights immediately after a single sample is misclassified.

  * **Initial Weights:** $a = (1, 0, 0)^T$.
  * **Learning Rate:** $\eta = 1$.
  * **Update Rule:** If misclassified, $a \leftarrow a + \eta \omega_k y_k$ (where $\omega_k$ is the class label: $1$ or $-1$).
  * **Training Data (Augmented):**

| Sample | Augmented $y^T$ | Class Label $\omega$ |
| :----- | :-------------- | :------------------- |
| 1      | (1, 0, 2)       | 1                    |
| 2      | (1, 1, 2)       | 1                    |
| 3      | (1, 2, 1)       | 1                    |
...
1.  **Sample 1:** $y=(1, 0, 2)$, Target=1.
    $g(x) = (1)(1) + (0)(0) + (0)(2) = 1$.
    Prediction is positive (Correct). No Update.

2.  **Sample 2:** $y=(1, 1, 2)$, Target=1.
    $g(x) = (1)(1) + (0)(1) + (0)(2) = 1$.
    Prediction is positive (Correct). No Update.

3.  **Sample 3:** $y=(1, 2, 1)$, Target=1.
    $g(x) = (1)(1) + (0)(2) + (0)(1) = 1$.
    Prediction is positive (Correct). No Update.

4.  **Sample 4:** $y=(1, -3, 1)$, Target=-1.
    $g(x) = (1)(1) + (0)(-3) + (0)(1) = 1$.
    Prediction is positive ($>0$), but target is negative ($-1$). **Misclassified**.
    **Update:** $a \leftarrow a + \eta \omega y$.
    $$a \leftarrow (1, 0, 0) + (1)(-1)(1, -3, 1)$$
    $$a \leftarrow (1, 0, 0) - (1, -3, 1)$$
    $$a \leftarrow (0, 3, -1)$$

5.  **Sample 5:** $y=(1, -2, -1)$, Target=-1.
    **Note:** We use the *new* weights $a=(0, 3, -1)$.
    $g(x) = (0)(1) + (3)(-2) + (-1)(-1) = 0 - 6 + 1 = -5$.
    Prediction is negative (Correct for Class -1). No Update.

6.  **Sample 6:** $y=(1, -3, -2)$, Target=-1.
    $g(x) = (0)(1) + (3)(-3) + (-1)(-2) = 0 - 9 + 2 = -7$.
    Prediction is negative (Correct for Class -1). No Update.
...
To understand how the cost function works, let's calculate the error for a specific set of weights and data points. We will use the data from the Widrow-Hoff (MSE) exercise in the tutorials.

**Question:**
Consider a single neuron with weights $a = [1, 0, 0]^T$ (augmented vector including bias).
Calculate the squared error contribution for the first two samples of the following dataset, where the target output (margin $b$) is set to 2.

  * **Sample 1:** Augmented input $y_1 = [1, 0, 2]^T$, Target $t_1 = 2$.
  * **Sample 2:** Augmented input $y_2 = [1, 1, 2]^T$, Target $t_2 = 2$.

**Step-by-Step Calculation:**

1.  **Calculate the Net Input ($s$) for Sample 1:**
    $$s_1 = a^T y_1 = (1 \times 1) + (0 \times 0) + (0 \times 2)$$
    $$s_1 = 1$$

2.  **Calculate the Squared Error for Sample 1:**
    $$E_1 = \frac{1}{2} (t_1 - s_1)^2$$
    $$E_1 = \frac{1}{2} (2 - 1)^2 = \frac{1}{2} (1)^2$$
    $$E_1 = 0.5$$

3.  **Calculate the Net Input ($s$) for Sample 2:**
    $$s_2 = a^T y_2 = (1 \times 1) + (0 \times 1) + (0 \times 2)$$
    $$s_2 = 1$$

4.  **Calculate the Squared Error for Sample 2:**
    $$E_2 = \frac{1}{2} (t_2 - s_2)^2$$
    $$E_2 = \frac{1}{2} (2 - 1)^2 = 0.5$$
...
To illustrate how the gradient guides the weights, let's calculate the gradient vector for the first sample used in Section 4.1.

**Question:**
Consider the linear neuron from the previous example.

  * **Weights:** $w = [1, 0, 0]^T$.
  * **Sample 1:** Augmented input $y_1 = [1, 0, 2]^T$.
  * **Target:** $t_1 = 2$.

Calculate the gradient of the squared error with respect to the weights for this single sample. Then, determine the direction of steepest descent.

**Step-by-Step Calculation:**

1.  **Recall the Net Input ($s$) and Error:**
    From Section 4.1, we calculated:

      * Net input $s_1 = 1$.
      * Target $t_1 = 2$.
      * Difference $(t_1 - s_1) = 1$.

2.  **Define the Gradient Formula:**
    The derivative of the error $E = \frac{1}{2}(t - s)^2$ with respect to the weights is:
    $$\nabla E = -(t - s)y$$
    *(Note: We calculate this for the augmented vector $y$. This derivation is detailed in the next section, but we apply the formula here)*.

3.  **Calculate the Gradient Vector ($\nabla E$):**
    Substitute the values:
    $$\nabla E = -(2 - 1) \begin{pmatrix} 1 \\ 0 \\ 2 \end{pmatrix}$$
    $$\nabla E = -1 \begin{pmatrix} 1 \\ 0 \\ 2 \end{pmatrix} = \begin{pmatrix} -1 \\ 0 \\ -2 \end{pmatrix}$$

    This vector $[-1, 0, -2]^T$ points **uphill**; this is the direction that would *increase* the error.

4.  **Determine Steepest Descent:**
    To reduce the error, we move in the opposite direction (negative gradient):
    $$-\nabla E = \begin{pmatrix} 1 \\ 0 \\ 2 \end{pmatrix}$$
...
Train a perceptron for one epoch using the delta rule.
  ...
  * **Learning Rate ($\alpha$):** 0.5.
  * **Training Example:** Augmented input $x = [1, 1, 2]^T$ (derived from a sample where $x_1=1, x_2=2$ or similar feature ordering), Target $t = 1$.

*(Note: This corresponds to the first example in Tutorial 6 Question 2, using the parameters established in Question 1.)*
...
**Question:**
Train a perceptron for one epoch using the generalised delta rule and stochastic gradient descent.

  * **Initial Weights:** $w_1=1$, $w_2=2$, and bias weight $w_0=1$.
  * **Learning Rate ($\alpha$):** 0.5.
  * **Training Example:** Feature vector $x = [1, 2]^T$ with Target $t = 1$. The augmented input vector is $x_{aug} = [1, 1, 2]^T$ (bias input is 1).
...
3.  **Calculate the Derivative Component:**
    ...
    $$g'(s) \approx 0.998 \times (1 - 0.998) = 0.998 \times 0.002$$
...
5.  **Update the Weights:**
    ...
    $$\Delta = 0.5 \times (0.002) \times (0.998 \times 0.002) \approx 0.000002$$

      * **Bias Weight ($w_0$):**
        $$w_0 \leftarrow 1 + (0.000002 \times 1) = 1.000002$$
      * **Weight 1 ($w_1$):**
        $$w_1 \leftarrow 1 + (0.000002 \times 1) = 1.000002$$
      * **Weight 2 ($w_2$):**
        $$w_2 \leftarrow 2 + (0.000002 \times 2) = 2.000004$$
...
1.  **Calculate Net Input ($s$):**
    We compute the dot product of the current weights and the new input vector.
...
2.  **Calculate Error ($t - s$):**
    $$Error = 0 - (-9) = 9$$
    (Note: The linear output is -9, but the target is 0; this results in a large positive error).

3.  **Update Weights:**
    Formula: $w_{new} = w_{old} + \alpha (t - s) x$.
    We calculate the adjustment vector first:
    $$Adjustment = 0.5 \times 9 \times [1, 3, 1]^T = 4.5 \times [1, 3, 1]^T = [4.5, 13.5, 4.5]^T$$

    Now we add this to the old weights:

      * **Bias ($w_0$):** $-1.5 + 4.5 = 3$
      * **Weight 1 ($w_1$):** $-1.5 + 13.5 = 12$
      * **Weight 2 ($w_2$):** $-3 + 4.5 = 1.5$

**Result:**
The new weight vector is $[3, 12, 1.5]^T$. The weights have changed significantly because the error on the linear output was large.

**Example 2: Generalised Delta Rule (Sigmoid Units)**

We now continue the non-linear example from Section 4.4. We use the sigmoid transfer function $g(s) = \frac{1}{1+e^{-s}}$.

**Current State (End of Sample 1):**

  * **Weights:** $w \approx [1.0, 1.0, 2.0]^T$ (Using rounded values for clarity; precise values are slightly higher).
  * **Learning Rate ($\alpha$):** 0.5.
...
1.  **Calculate Net Input ($s$):**
    $$s = (1.0 \times 1) + (1.0 \times 3) + (2.0 \times 1)$$
    $$s = 6.0$$
...
3.  **Calculate Gradient Components:**
      ...
      * **Derivative ($g(s)(1 - g(s))$):** $0.998 \times (1 - 0.998) \approx 0.002$.

4.  **Update Weights:**
    Formula: $w_{new} = w_{old} + \alpha (t - g(s)) g'(s) x$.
    Scalar multiplier: $\Delta = 0.5 \times (-0.998) \times 0.002 \approx -0.001$.

      * **Bias ($w_0$):** $1.0 + (-0.001 \times 1) = 0.999$.
      * **Weight 1 ($w_1$):** $1.0 + (-0.001 \times 3) = 0.997$.
      * **Weight 2 ($w_2$):** $2.0 + (-0.001 \times 1) = 1.999$.
