...
describe("Reader Component", () => {
  ...
  test("should render standard markdown elements with typeset wrapper and slug id", () => {
    const markdown =
    ...
    expect(html).toContain(
      '<h1 id="test-heading" class="markdown-heading">Test Heading</h1>',
    );
    expect(html).toContain("<strong>bold</strong>");
    expect(html).toContain("<em>italic</em>");
  });

  test("should generate proper slug id for complex numbered headings matching TOC links", () => {
    const markdown = "## 10.1 - Summary of Work";
    const html = renderToStaticMarkup(<Reader content={markdown} />);

    expect(html).toContain('id="101---summary-of-work"');
  });
...
| Cell 1 | Cell 2 |
  ...
  test("should preserve inline code spans and not confuse with math delimiters", () => {
    const codeMarkdown =
      "Use `$foo` and `$$bar` to declare variables in shell.";
    const html = renderToStaticMarkup(<Reader content={codeMarkdown} />);

    expect(html).toContain('<code class="inline-code">$foo</code>');
    expect(html).toContain('<code class="inline-code">$$bar</code>');
  });
});
